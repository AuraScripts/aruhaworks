  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mode === "cart") {
          // Return to the shop and open the cart drawer with the new item.
          window.location.href = "/?cartOpen=1";
          return;
        }

        // Buy-now flow — return and open the Tebex checkout overlay.
...
export default function CheckoutContinue() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center px-6 text-center">
      <Suspense
        fallback={
          <p className="font-mono text-xs uppercase tracking-widest2 text-dim">Loading…</p>
        }
      >
        <ContinueContent />
      </Suspense>
