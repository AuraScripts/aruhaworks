...
export default function ProductCard({
  ...
  const originalPrice = hasDiscount
    ? pkg.base_price + (pkg.base_price * pkg.discount) / (100 - pkg.discount)
    : null;
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {tags.map((t) => (
              <span key={t} className="framework-tag">
                {t}
              </span>
            ))}
          </div>
        ...
        <Link href={`/product/${pkg.id}`}>
          <h3 className="line-clamp-2 font-display text-base font-semibold leading-snug text-text transition group-hover:text-accent">
            {pkg.name}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-1">
          ...
          <button
            ...
            className="rounded-lg bg-accent px-3.5 py-2 text-xs font-semibold text-white transition hover:bg-accentHover"
          >
            Add to Basket
          </button>
        </div>
